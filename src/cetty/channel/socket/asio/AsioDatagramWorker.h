...
class NioDatagramWorker implements Runnable {
    ...
    void register(final NioDatagramChannel channel, final ChannelFuture future) {
        ...
        synchronized (startStopLock) {
            ...
            bool offered = registerTaskQueue.offer(channelRegTask);
            assert offered;
        }
    ...
    private bool cleanUpCancelledKeys() throws IOException {
     ...
     */
    private bool read(final SelectionKey key) {
    ...
    private bool scheduleWriteIfNecessary(final NioDatagramChannel channel) {
        final Thread workerThread = thread;
        if (workerThread == null || Thread.currentThread() != workerThread) {
            if (channel.writeTaskInTaskQueue.compareAndSet(false, true)) {
                // "add" the channels writeTask to the writeTaskQueue.
                bool offered = writeTaskQueue.offer(channel.writeTask);
                assert offered;
            }
