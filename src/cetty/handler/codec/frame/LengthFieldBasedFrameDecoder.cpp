...
LengthFieldBasedFrameDecoder::decode(ChannelHandlerContext& ctx,
    ...
    if (initialBytesToStrip > frameLengthInt) {
        ...
        throw CorruptedFrameException(
                std::string("Adjusted frame length (") +
                Integer::toString(frameLength) +
                std::string(") is less than initialBytesToStrip: ") +
...
void LengthFieldBasedFrameDecoder::fail(ChannelHandlerContext& ctx, int frameLength) {
    ...
    if (frameLength > 0) {
        msg.append("Adjusted frame length exceeds ");
...
void LengthFieldBasedFrameDecoder::validateParameters() {
    ...
    if (lengthFieldOffset > maxFrameLength - lengthFieldLength) {
        std::string msg;
        msg.reserve(256);
        msg.append("maxFrameLength (");
        Integer::append(msg, maxFrameLength);
        msg.append(") must be equal to or greater than lengthFieldOffset (");
        Integer::append(msg, lengthFieldOffset);
        msg.append(") + lengthFieldLength (");
        Integer::append(msg, lengthFieldLength);
        msg.append(").");
