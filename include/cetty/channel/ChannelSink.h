...
public:
     ...
     */
    virtual void eventSunk(const ChannelPipeline& pipeline, const ChannelEvent& e) = 0;

    /**
     * Invoked by {@link ChannelPipeline} when a downstream {@link MessageEvent}
     * has reached its terminal (the head of the pipeline).
     */
    virtual void writeRequested(const ChannelPipeline& pipeline, const MessageEvent& e) = 0;

    /**
     * Invoked by {@link ChannelPipeline} when a downstream {@link ChannelStateEvent}
     * has reached its terminal (the head of the pipeline).
     */
    virtual void stateChangeRequested(const ChannelPipeline& pipeline, const ChannelStateEvent& e) = 0;
