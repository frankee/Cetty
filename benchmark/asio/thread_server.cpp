...
public:
  ...
  void start()
  {
    system::error_code set_option_err;
  ...
  void handle_read(const system::error_code& err, size_t length)
  {
    --op_count_;
  ...
  void handle_write(const system::error_code& err)
  {
    --op_count_;
...
{
public:
  server(const asio::ip::tcp::endpoint& endpoint, size_t block_size, size_t thread_count)
      ...
      block_size_(block_size)
  {
    ...
    session* new_session = new session(io_service_pool_.get_io_service(), block_size_);
    acceptor_.async_accept(new_session->socket(),
        boost::bind(&server::handle_accept, this, new_session,
          asio::placeholders::error));
  }

  void handle_accept(session* new_session, const system::error_code& err)
  {
    if (!err)
    {
      new_session->start();
      new_session = new session(io_service_pool_.get_io_service(), block_size_);
      acceptor_.async_accept(new_session->socket(),
          boost::bind(&server::handle_accept, this, new_session,
            asio::placeholders::error));
    }
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    server s(asio::ip::tcp::endpoint(address, port), block_size, thread_count);
    s.run();
  }
