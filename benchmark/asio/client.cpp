...
public:
  ...
  {
    asio::detail::mutex::scoped_lock lock(mutex_);
    std::cout << "total written " << std::setw(16) << total_bytes_written_;
    std::cout << "\ttotal read "<< std::setw(16) << total_bytes_read_;
    if (timeout) {
        std::cout << "\twritten(BPS) " << std::setw(16) << total_bytes_written_ / timeout;
        std::cout << "\tread(BPS) "<< std::setw(16) << total_bytes_read_ / timeout;
    }
    std::cout << "\n";
  }
...
private:
  ...
  void handle_write(const system::error_code& err, size_t length)
  {
    if (!err && length > 0)
    {
